...
import { RefreshCw, Plus, Eye, Calendar, Trash2, TrendingDown } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 rounded-xl border bg-card">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-muted-foreground text-sm">Tổng phiếu xuất</span>
          </div>
          ...
          groupedGINs.map((group, groupIndex) => (
            <div key={group.date} className="space-y-2">
              ...
              <div className="flex items-center gap-3 py-2">
                ...
                <Badge variant="secondary" className="flex items-center gap-1.5 bg-orange-100 text-orange-700">
                  {group.gins.length} phiếu
                  {canViewValues && (
                    <>
                      <span className="text-muted-foreground">•</span>
                      <CurrencyDisplay value={group.totalValue} size="sm" variant="expense" compact />
                    </>
                  )}
                </Badge>
              </div>
                    ...
                    {group.gins.map((gin) => {
                      ...
                      <TableRow key={gin.id}>
                        ...
                        <TableCell className="text-right">
                          <CurrencyDisplay value={Number(gin.total_value)} size="sm" variant="expense" hideValue={!canViewValues} />
                        </TableCell>
                        <TableCell>
