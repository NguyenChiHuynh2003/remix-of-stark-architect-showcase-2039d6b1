  ...
  const parseExcelFile = async (file: File) => {
    ...
    try {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        // Tính toán
        const baseValue = inbound_quantity * inbound_value;
        ...
        // Cuối kỳ GT = Base value × 1.12
        closing_value = Math.round(baseValue * INTERNAL_FACTOR);
        ...
        // Tính VAT
        if (vat_rate > 0) {
          vat_value = Math.round(closing_value * vat_rate);
          total_with_vat = closing_value + vat_value;
        } else {
          vat_value = 0;
          total_with_vat = closing_value;
        }
