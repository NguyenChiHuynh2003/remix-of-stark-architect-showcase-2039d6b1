...
const detectAssetType = (name: string): { type: AssetType; consumable: boolean } => {
  ...
  const materialsKeywords = [
    "băng keo", "dây", "ốc", "vít", "đai", "silicon", "keo", "seal", "cáp",
    "dây điện", "ống", "terminal", "đầu cos", "domino", "bushing", "gen co",
    "tấm", "lá", "cuộn", "phim", "thùng", "can", "bình", "túi", "hộp",
    "cell pin", "cát", "xi măng", "gạch", "sơn", "móc", "ke", "thanh nhôm"
  ];
  
  // Equipment keywords
  const equipmentKeywords = [
    "máy", "thiết bị", "đồng hồ", "ampe", "hioki", "camera", "inverter",
    "biến tần", "tủ", "monitor", "ir4053", "fluke", "multimeter"
  ];
  
  // Tools keywords
  const toolsKeywords = [
    "kềm", "kìm", "cà lê", "tuốc nơ vít", "búa", "cưa", "khoan", "điếu",
    "lục giác", "típ", "vít", "cờ lê", "bộ đầu", "mũi khoan", "lưỡi cắt",
    "thước", "dao", "đèn pin", "kính bảo hộ", "áo bảo hộ", "nón", "găng tay"
  ];
...
export function AssetImportDialog({ open, onClose }: AssetImportDialogProps) {
  ...
  const parseExcelFile = async (file: File) => {
    ...
    try {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        if (firstCell.toLowerCase().includes("tên kho") || firstCell.toLowerCase().includes("ten kho")) {
          ...
          if (warehouseMatch) {
            ...
            currentWarehouse = firstCell.replace(/Tên kho\s*:\s*/i, "").replace(/\s*\(\d+\s*\)\s*$/, "").trim();
          }
          continue;
        }
      ...
      warehouseMap.forEach((warehouseAssets, warehouseName) => {
        const totals = warehouseAssets.reduce((acc, asset) => ({
  ...
  const getAssetTypeIcon = (type: AssetType) => {
    switch (type) {
      case "equipment": return <Package className="h-3 w-3" />;
      case "tools": return <Wrench className="h-3 w-3" />;
      case "materials": return <Hammer className="h-3 w-3" />;
    }
  };
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Tải lên file Excel báo cáo tồn kho để nhập dữ liệu tài sản vào hệ thống
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <h4 className="font-medium text-sm">
                  Xem trước dữ liệu ({validCount} tài sản hợp lệ)
                </h4>
                  ...
                  <TableBody>
                    {parsedData.map((asset, index) => (
                      <TableRow key={index} className={!asset.isValid ? "opacity-50" : ""}>
                        ...
                        <TableCell className="text-right text-xs font-medium">{formatNumber(asset.closing_quantity)}</TableCell>
                        <TableCell className="text-right text-xs font-medium">{formatNumber(asset.closing_value)}</TableCell>
                        <TableCell>
                          <Select 
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              disabled={loading || importing || validCount === 0}
            >
