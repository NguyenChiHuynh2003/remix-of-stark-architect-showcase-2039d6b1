 ...
 export async function generateUserGuidePDF() {
   ...
   y += 10;
   addTitle("HỆ THỐNG QUẢN LÝ DOANH NGHIỆP", 20);
   y += 30;
   ...
   y += 8;
   doc.text(`Ngày cập nhật: ${new Intl.DateTimeFormat("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric" }).format(new Date())}`, pageWidth / 2, y, { align: "center" });
   ...
   // ========== CÀI ĐẶT ==========
   addNewPage();
   addSectionTitle("6. MODULE CÀI ĐẶT");
   addDivider();
   
   addSubTitle("6.1 Sơ đồ Tổ chức");
   addParagraph("Xây dựng và quản lý sơ đồ cơ cấu tổ chức của doanh nghiệp.");
   addBulletPoint("Thêm các vị trí/chức danh trong tổ chức");
   addBulletPoint("Liên kết vị trí với nhân viên cụ thể");
   addBulletPoint("Thiết lập mối quan hệ cấp trên - cấp dưới");
   addBulletPoint("Kéo thả để sắp xếp vị trí trên sơ đồ");
 
   y += 5;
   addSubTitle("6.2 Sao lưu & Khôi phục");
   addParagraph("Quản lý sao lưu dữ liệu hệ thống.");
   addBulletPoint("Thiết lập lịch sao lưu tự động");
   addBulletPoint("Sao lưu thủ công khi cần");
   addBulletPoint("Khôi phục dữ liệu từ bản sao lưu");
 
   // ========== QUẢN LÝ NGƯỜI DÙNG ==========
   y += 10;
   addSectionTitle("7. QUẢN LÝ NGƯỜI DÙNG");
   addDivider();
   addParagraph("(Chỉ dành cho Quản trị viên)");
   y += 3;
   
   addSubTitle("7.1 Tạo tài khoản mới");
   ...
   addSubTitle("7.2 Phân quyền truy cập");
   ...
   addSubTitle("8.3 Các báo cáo có sẵn");
   ...
   // ========== LIÊN HỆ HỖ TRỢ ==========
   y += 15;
   addSectionTitle("LIÊN HỆ HỖ TRỢ");
   addDivider();
   addParagraph("Nếu bạn gặp vấn đề trong quá trình sử dụng, vui lòng liên hệ:");
   addBulletPoint("Email: support@company.com");
   addBulletPoint("Điện thoại: 0123 456 789");
   addParagraph("Đội ngũ hỗ trợ sẽ phản hồi trong vòng 24 giờ làm việc.");
